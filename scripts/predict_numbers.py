...
def train_lstm_model(series):
    ...
    def generate_sequences(data, look_back):
        return [(data[i:i + look_back], data[i + look_back]) for i in range(len(data) - look_back)]
    ...
    model = Sequential([
        ...
        LSTM(16, return_sequences=False),  # Fewer LSTM units
        Dense(1)
    ])
...
def analyze_and_predict():
    ...
    number_counts = pd.Series(df[num_cols].values.flatten()).value_counts().sort_index()
